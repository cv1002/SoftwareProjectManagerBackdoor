...
CREATE UNIQUE INDEX XPKCommunication ON Communication
    (
     CommunicationID
        );
...
CREATE UNIQUE INDEX XPKFileAssess ON FileAssess
    (
     FileID
        );
...
CREATE UNIQUE INDEX XPKFiles ON Files
    (
     FileID
        );
...
CREATE UNIQUE INDEX XPKGroupAssess ON GroupAssess
    (
     TeamID
        );
...
CREATE UNIQUE INDEX XPKMilestone ON Milestone
    (
     MilestoneID
        );
...
CREATE UNIQUE INDEX XPKPermissionConfig ON PermissionConfig
    (
     ConfigID
        );
...
CREATE UNIQUE INDEX XPKProjectAssignment ON ProjectAssignment
    (
     TeamID
        );
...
CREATE UNIQUE INDEX XPKProjectCompletion ON ProjectCompletion
    (
     CompletionID
        );
...
CREATE UNIQUE INDEX XPKRole ON Role
    (
     RoleID
        );
...
CREATE UNIQUE INDEX XPKRolePowerAssignment ON RolePowerAssignment
    (
     AssignmentID
        );
...
CREATE UNIQUE INDEX XPKStudentAssess ON StudentAssess
    (
     StudentUserID
        );
...
ALTER TABLE Task
    ADD FOREIGN KEY R_14 (TaskHandlerID) REFERENCES Student (StudentUserID);
